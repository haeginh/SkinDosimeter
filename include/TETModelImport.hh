...
{
public:
 TETModelImport(G4String GetPhantomName, G4UIExecutive* ui);
 ...
 G4int         GetNumSkinDet()            { return -fVolumeMap.begin()->first-1;}
 ...
 std::map<G4int, std::vector<G4int>> GetOrgan2Dose() {return organ2dose;}
 std::vector<G4double> GetDoseMass() {return doseMass;}
 std::vector<G4String> GetDoseName() {return doseName;}
 std::vector<G4double> GetEffCoeff() {return effCoeff;}
...
private:
 ...
 std::vector<G4String>  doseName;
 std::vector<G4double>  doseMass;
 std::vector<G4double>  effCoeff;
};

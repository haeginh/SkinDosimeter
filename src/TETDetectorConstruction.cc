...
void TETDetectorConstruction::SetupWorldGeometry()
{
 ...
 //
 auto* containerSolid = new G4Box("phantomBox", fPhantomSize.x()*0.5 + 10.*cm,
					           fPhantomSize.y()*0.5 + 10.*cm,
						   fPhantomSize.z()*0.5 + 10.*cm);

 fContainer_logic = new G4LogicalVolume(containerSolid, vacuum, "phantomLogical");

 new G4PVPlacement(nullptr, (fPhantomBoxMin+fPhantomBoxMax)*0.5, fContainer_logic, "PhantomPhysical",
...
void TETDetectorConstruction::ConstructSDandField()
{
 ...
 MFDet->RegisterPrimitive(new TETPSSkinDosimeter("skin", fTetData));
 MFDet->RegisterPrimitive(new DRFScorer("drf", fTetData));

                                                 ...
                                                 const G4VTouchable* )
{
  ...
  G4Material* mat;
  if(idx>0) mat = fTetData->GetMaterial(idx);
  else if(idx&1) mat = fTetData->GetMaterial(12201); // skin target
  else mat = fTetData->GetMaterial(12200); // skin non-target

  if(fIsforVis){
	  if(idx>0) phy->GetLogicalVolume()->SetVisAttributes(fVisAttMap[idx]);
    else if(idx&1) phy->GetLogicalVolume()->SetVisAttributes(fVisAttMap[12201]);
    else phy->GetLogicalVolume()->SetVisAttributes(fVisAttMap[12200]);

	  phy->GetLogicalVolume()->SetMaterial(mat);
	}
